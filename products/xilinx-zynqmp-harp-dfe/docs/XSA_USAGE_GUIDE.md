...
## XSA File Location
```
Path: products/xilinx-zynqmp-harp-dfe/hardware/system.xsa  (relative to repo root)
```
...
These settings live together in `products/xilinx-zynqmp-harp-dfe/conf/local.conf.fragment`:
...
The reusable Xilinx platform adapter selects `XILINX_WITH_ESW = "xsct"`; the product fragment owns only the concrete XSA path and artifact policy.
...
2. Copy to project location:
   ```bash
   cp /path/to/new_design.xsa products/xilinx-zynqmp-harp-dfe/hardware/system.xsa
   ```

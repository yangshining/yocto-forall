...
位置：products/xilinx-zynqmp-harp-dfe/hardware/system.xsa  (repo root 相对路径)
```
...
cp <vivado_project>/design.runs/impl_1/system_wrapper.bit \
   products/xilinx-zynqmp-harp-dfe/hardware/system.bit
```
...
```bash
cd products/xilinx-zynqmp-harp-dfe/hardware/
...
创建 recipe：`products/xilinx-zynqmp-harp-dfe/meta-xilinx-zynqmp-harp-dfe/recipes-bsp/fpga-manager/fpga-bitstream.bb`
...
放置 bitstream：
```bash
mkdir -p products/xilinx-zynqmp-harp-dfe/meta-xilinx-zynqmp-harp-dfe/recipes-bsp/fpga-manager/files/
cp system.bit.bin products/xilinx-zynqmp-harp-dfe/meta-xilinx-zynqmp-harp-dfe/recipes-bsp/fpga-manager/files/
```

添加到镜像：在 `products/xilinx-zynqmp-harp-dfe/conf/local.conf.fragment` 中添加：
```bitbake
...
修改：`products/xilinx-zynqmp-harp-dfe/meta-xilinx-zynqmp-harp-dfe/recipes-bsp/u-boot/files/boot.cmd`
...
修改：`products/xilinx-zynqmp-harp-dfe/meta-xilinx-zynqmp-harp-dfe/recipes-bsp/u-boot/u-boot-xlnx-scr.bbappend`
...
**解决：** 检查 U-Boot 配置
```bash
# 在 products/xilinx-zynqmp-harp-dfe/meta-xilinx-zynqmp-harp-dfe/recipes-bsp/u-boot/u-boot-xlnx_%.bbappend
UBOOT_FEATURES += "fpga"
```
